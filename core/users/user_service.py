...
def user_update_service(instance, validated_data):
    ...
    instance.save(
        update_fields=["username", "password", "email", "image_path", "title"]
    )
    return instance


def verify_email(email):
    response = ses_client.verify_email_identity(EmailAddress="max2121vas@gmail.com")
    return response
...
def mynews_posts(user):
    ...
    reset_queries()
    posts = Post.objects.filter(Q(page__in=my_pages) | Q(page__in=followed_pages))[:20]
    return posts
